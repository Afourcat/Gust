...
use crate::draw::{Drawable, Movable};
use crate::Vector;
...
impl Drawable for Shapes {
    fn draw<T: Drawer>(&self, window: &mut T) {
        match self {
            Shapes::Circle(r, shape) => println!("Circle {} {:?}", r, shape),
            Shapes::Square(d, shape) => println!("Square {} {:?}", d, shape),
            Shapes::Rectangle(w, h) => println!("Rectangle {} {} {:?}", w, h, shape),
            Shapes::Polygon(r, nbr) => println!("Polygon {} {} {:?}", r, nbr, shape),
        }
    }
